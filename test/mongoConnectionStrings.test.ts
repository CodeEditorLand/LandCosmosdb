...
function testDatabaseToAccountConnectionString(
	...
	const databaseConnectionString = addDatabaseToAccountConnectionString(
		connectionString,
		databaseName
	);
...
function testDatabaseNameFromConectionString(
	connectionString: string,
...
function testIsCosmosEmulatorConnectionString(
	connectionString: string,
	expected: boolean
): void {
...
function testEncodeMongoConnectionString(
	connectionString: string,
...
suite(`mongoCollectionStrings`, () => {
	test(`getDatabaseNameFromConnectionString`, () => {
		...
		testDatabaseNameFromConectionString(
			`mongodb://r1.example.net:27017,r2.example.net:27017/db`,
			"db"
		);
		...
		testDatabaseNameFromConectionString(
			`mongodb+srv://server.example.com/db`,
			"db"
		);
		...
		testDatabaseNameFromConectionString(
			`mongodb://localhost:${encodeURIComponent(
				emulatorPassword
			)}@localhost:10255/admin?ssl=true`,
			"admin"
		);
		testDatabaseNameFromConectionString(
			`mongodb://localhost:${encodeURIComponent(
				emulatorPassword
			)}@localhost:10255/admin-master?ssl=true`,
			"admin-master"
		);
		...
		testDatabaseNameFromConectionString(
			`mongodb://localhost:${encodeURIComponent(
				emulatorPassword
			)}@localhost:10255/admin!@%^()-_,[]?ssl=true`,
			"admin!@%^()-_,[]"
		);
	});
	...
	test("addDatabaseToAccountConnectionString", () => {
		...
		testDatabaseToAccountConnectionString(
			...
			`mongodb://localhost:${encodeURIComponent(
				emulatorPassword
			)}@localhost:10255/${encodeURIComponent(
				"admin!@%^()-_,[]"
			)}?ssl=true`
		);
	});
	...
	test("isCosmosEmulatorConnectionString", () => {
		...
		testIsCosmosEmulatorConnectionString(
			`mongodb://127.0.0.1:${encodeURIComponent(
				emulatorPassword
			)}@127.0.0.1:10255/?ssl=true`,
			true
		);
		testIsCosmosEmulatorConnectionString(
			`mongodb://localhost:${encodeURIComponent(
				emulatorPassword
			)}@localhost:10255/database?ssl=true`,
			true
		);
	});
	...
	test("encodeMongoConnectionString", () => {
		...
		testEncodeMongoConnectionString(
			...
			`mongodb://localhost:${encodeURIComponent(
				encodeURIComponent(emulatorPassword)
			)}@localhost:10255/?ssl=true`
		);
		testEncodeMongoConnectionString(
