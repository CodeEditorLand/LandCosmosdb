...
suite("isIpInRanges", function () {
	...
	test("Includes ip at start", function () {
		const ranges = [
			{ startIpAddress: "12.34.56.78", endIpAddress: "12.34.56.80" },
		];
		assert(isIpInRanges(ip, ranges));
	});
	test("Includes ip at end", function () {
		const ranges = [
			{ startIpAddress: "12.34.56.76", endIpAddress: "12.34.56.78" },
		];
		assert(isIpInRanges(ip, ranges));
	});
	test("Includes ip in range", function () {
		const ranges = [
			{ startIpAddress: "12.34.56.76", endIpAddress: "12.34.56.80" },
		];
		assert(isIpInRanges(ip, ranges));
	});
	test("Excludes ip before start", function () {
		const ranges = [
			{ startIpAddress: "12.34.56.80", endIpAddress: "12.34.56.80" },
		];
		assert(!isIpInRanges(ip, ranges));
	});
	test("Excludes ip after end", function () {
		const ranges = [
			{ startIpAddress: "12.34.56.76", endIpAddress: "12.34.56.76" },
		];
		assert(!isIpInRanges(ip, ranges));
	});
});
