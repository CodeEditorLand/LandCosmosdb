...
suite("MongoShell", async function (this: Mocha.Suite): Promise<void> {
	...
	async function testIfSupported(
		title: string,
		fn?: Mocha.Func | Mocha.AsyncFunc
	): Promise<void> {
		await runWithSetting(
			ext.settingsKeys.mongoShellTimeout,
			"60",
			async () => {
				if (testsSupported) {
					test(title, fn);
	...
	if (!fse.existsSync(mongodPath)) {
		console.log(
			`Couldn't find mongod.exe at ${mongodPath} - skipping MongoShell tests`
		);
		testsSupported = false;
	} else if (!fse.existsSync(mongodPath)) {
		console.log(
			`Couldn't find mongo.exe at ${mongoPath} - skipping MongoShell tests`
		);
		testsSupported = false;
	} else {
	...
	suiteSetup(() => {
		if (testsSupported) {
			assert(
				fse.existsSync(mongodPath),
				"Couldn't find mongod.exe at " + mongodPath
			);
			assert(
				fse.existsSync(mongoPath),
				"Couldn't find mongo.exe at " + mongoPath
			);
	...
	await testIfSupported("Verify mongod running", async () => {
		while (
			!mongoDOutput.includes("waiting for connections on port 27017")
		) {
		...
		await testIfSupported(options.title || options.script, async () => {
			...
			try {
				...
				if (options.expectedError) {
					assert(
						false,
						`Expected error did not occur: '${options.expectedError}'`
					);
				}
				...
				if (options.expectedError instanceof RegExp) {
					...
					assert(
						false,
						`Unexpected error during the test: ${message}`
					);
				}
	...
	await testIfSupported(
		"More results than displayed (type 'it' for more -> (More))",
		async () => {
			const shell = await MongoShell.create(
				mongoPath,
				[],
				"",
				false,
			...
			await shell.executeScript(
				"for (var i = 0; i < 50; ++i) { db.mongoShellTest.insert({a:i}); }"
			);

			const result = await shell.executeScript(
				"db.mongoShellTest.find().pretty()"
			);
