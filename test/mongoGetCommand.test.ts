	...
	function testCore(coreText: string): void {
		...
		if (expectedCommand) {
			...
			assert.equal(
				...
				"Parsed command name is not correct"
			);

			const actualArgs = (command.arguments || []).map((arg) =>
				JSON.parse(arg)
			);
			assert.deepEqual(
				actualArgs,
				expectedCommand.args || [],
				"Parsed arguments are not correct"
			);
		} else {
...
suite("scrapbook parsing Tests", () => {
	test("find", () => {
		...
		const command: MongoCommand = findCommandAtPosition(
			commands,
			new Position(0, 0)
		);
	...
	test("test function call with and without quotes", () => {
		for (let q = 0; q <= 2; q++) {
			const text = `db.test1.insertMany({${wrapInQuotes(
				"name",
				q
			)}: 'First' })`;
			const commands: MongoCommand[] = getAllCommandsFromText(text);
	...
	test("test aggregate query", () => {
		for (let q = 0; q <= 2; q++) {
			const text = `db.orders.aggregate([
                { ${wrapInQuotes("$match", q)}: { ${wrapInQuotes(
					"status",
					q
				)} : "A" } },
                { ${wrapInQuotes("$group", q)}: { ${wrapInQuotes(
					"_id",
					q
				)}: "$cust_id", ${wrapInQuotes("total", q)}: { ${wrapInQuotes(
					"$sum",
					q
				)}: "$amount" } } },
                { ${wrapInQuotes("$sort", q)}: { ${wrapInQuotes(
					"total",
					q
				)}: -1 } }
                ],
                {
                    ${wrapInQuotes("cursor", q)}: { ${wrapInQuotes(
						"batchSize",
						q
					)}: 0 }
                })`;
	...
	test("test ObjectID - no parameter", () => {
		...
		assert.ok(
			(<any>nonNullProp(command, "argumentObjects")[0]).name instanceof
				ObjectID
		);
	});
	...
	test("test faulty ObjectID - hex - fewer characters", () => {
		...
		for (let i = 1; i < 3; i++) {
			const text = `db.c1.insert({"name": ObjectId(${wrapInQuotes(
				idParam,
				i
			)})})`;
			const commands: MongoCommand[] = getAllCommandsFromText(text);
	...
	test("test regular expressions - only pattern, no flags", () => {
		...
		const generatedRegExp = (<any>(
			nonNullProp(command, "argumentObjects")[0]
		)).sku;
	...
	test("test regular expressions - wrong escape - throw error", () => {
		...
		try {
			...
			assert.equal(
				error.message,
				"Invalid regular expression: /789$\\/b\\/: \\ at end of pattern"
			);
		}
	});
	...
	test("test regular expression parsing interoperability", () => {
		...
		const command1: MongoCommand = findCommandAtPosition(
			commands1,
			new Position(0, 0)
		);
		const generatedRegExp1 = (<any>(
			nonNullProp(command1, "argumentObjects")[0]
		)).sku;
		...
		const command2: MongoCommand = findCommandAtPosition(
			commands2,
			new Position(0, 0)
		);
		const generatedRegExp2 = (<any>(
			nonNullProp(command2, "argumentObjects")[0]
		)).sku;
		assert.deepEqual(
			[generatedRegExp1.flags, generatedRegExp1.source],
			["g", "^(hello?= world).*[^0-9]+|(world\\b\\*){0,2}$"]
		);
		assert.deepEqual(
			[generatedRegExp2.flags, generatedRegExp2.source],
			["g", "^(hello?= world).*[^0-9]+|(world\\b\\*){0,2}$"]
		);
	});
	...
	test("test regular expressions - Intellisense - flag contains invalid option", () => {
		...
		assert.deepEqual(
			nonNullProp(command, "errors")[0].message,
			"Invalid flags supplied to RegExp constructor 'q'"
		);
		assert.deepEqual(
			nonNullProp(command, "errors")[0].range.start.character,
			19
		);
	});
	...
	test("test chained command: argument aggregation", () => {
		testParse(
			'db.timesheets.find({name: "Andy", surname: "Jackson"}).pretty();',
			{
				collection: "timesheets",
				name: "pretty",
	...
	test("Chained command - order of parsed arguments", () => {
		testParse(
			'db.timesheets.find({name:"Andy"}).sort({age: 1}).skip(40);',
			{
				collection: "timesheets",
				name: "skip",
	...
	test("test user issues: https://github.com/Microsoft/vscode-cosmosdb/issues/703", () => {
		for (let q = 0; q <= 2; q++) {
			const text = `db.Users.find({ ${wrapInQuotes(
				"user",
				q
			)}: { ${wrapInQuotes(
				"$in",
				q
			)}: [ "A80", "HPA" ] } },{ ${wrapInQuotes("_id", q)}: false });`;
			const commands: MongoCommand[] = getAllCommandsFromText(text);
			const command: MongoCommand = findCommandAtPosition(
				commands,
				new Position(0, 0)
		...
		for (let q = 0; q <= 2; q++) {
			const text = `db.users.aggregate([
                { ${wrapInQuotes("$match", q)}: {${wrapInQuotes(
					"_id",
					q
				)}: {"$oid" :"5b23d2ba92b52cf794bdeb9c")}}},
                { ${wrapInQuotes("$project", q)}: {
                    ${wrapInQuotes("scores", q)}: {${wrapInQuotes(
						"$filter",
						q
					)}: {
                        ${wrapInQuotes("input", q)}: '$scores',
                        ${wrapInQuotes("as", q)}: 'score',
                        ${wrapInQuotes("cond", q)}: {${wrapInQuotes(
							"$gt",
							q
						)}: ['$$score', 3]}
                    }}
