...
export const extractPartitionKey = (
...
export const getDocumentId = (
	...
	const documentId = {
		_rid:
			typeof document["_rid"] === "string" ? document["_rid"] : undefined,
		id: document["id"],
		partitionKey: partitionKey
			? extractPartitionKey(document, partitionKey)
			: undefined,
	};
	...
	if (
		documentId._rid &&
		typeof documentId._rid === "string" &&
		documentId._rid.length > 0
	) {
		return documentId;
	}
	...
	if (partitionKey) {
		if (
			Array.isArray(documentId.partitionKey) &&
			documentId.partitionKey.some((key) => key === undefined)
		) {
			return undefined;
		}
