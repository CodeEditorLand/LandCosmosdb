...
export const queryResultToTree = (
	...
	queryResult.documents.forEach((doc, index) => {
		const documentTree = documentToSlickGridTree(
			doc,
			partitionKey,
			index,
			`${index}-`,
...
const documentToSlickGridTree = (
	...
	tree.push({
		...
		field: document["id"]
			? `${document["id"]}`
			: `${index + 1} (Index number, id is missing)`,
		value: "",
		type: "Document",
		parentId: null,
	});

	const stack: { key: string; value: unknown; parentId: string | null }[] =
...
export const queryResultToTable = (
	...
	if (reorderColumns) {
		result = {
			headers: getTableHeadersWithRecordIdentifyColumns(
				queryResult.documents,
				partitionKey,
			),
			dataset: getTableDatasetWithRecordIdentifyColumns(
				queryResult.documents,
				partitionKey,
			),
		};
...
export const queryMetricsToTable = (
	queryResult: SerializedQueryResult | null,
): StatsItem[] => {
	...
	const stats: StatsItem[] = [
		...
		},
		{
			...
			value:
				queryMetrics.runtimeExecutionTimes.queryEngineExecutionTime ??
				0,
...
export const queryMetricsToJSON = (
	queryResult: SerializedQueryResult | null,
): string => {
	if (!queryResult) {
		return "";
...
export const queryMetricsToCsv = (
	queryResult: SerializedQueryResult | null,
): string => {
	if (!queryResult) {
		return "";
	...
	const values = stats
		.map((item) => escapeCsvValue(item.value.toString()))
		.join(",");
...
export const queryResultToCsv = (
	...
	if (selection) {
		tableView.dataset = tableView.dataset.filter((_, index) =>
