...
export async function showNewFile(
	...
	const fullFileName: string | undefined = await getUniqueFileName(
		folderPath,
		fileName,
		fileExtension
	);
	const uri: vscode.Uri = vscode.Uri.file(
		path.join(folderPath, fullFileName)
...
function isAccountTreeItem(treeItem: AzExtTreeItem): boolean {
	return (
		...
		treeItem instanceof DocDBAccountTreeItemBase
	);
}

export function getDocumentTreeItemLabel(
	document: IMongoDocument | ItemDefinition
): string {
