...
export async function getConfirmationAsInSettings(
	...
	if (deleteConfirmation === ConfirmationStyle.wordConfirmation) {
		return await getConfirmationWithWordQuestion(
			title,
			message,
...
export async function getConfirmationWithNumberQuiz(
	title: string,
	message: string,
): Promise<boolean> {
	const randomInput: { numbers: number[]; index: number } =
		getRandomArrayAndIndex(3);
...
export async function getConfirmationWithClick(
	title: string,
	message: string,
): Promise<boolean> {
	const confirmation = await vscode.window.showWarningMessage(
