 ...
 */
export function documentToSlickGridTree(
	...
	tree.push({
		id: rootId,
		field:
			document._id._bsontype === "ObjectId"
	...
	while (stack.length > 0) {
		...
		const dataType: MongoBSONTypes = MongoBSONTypes.inferType(
			stackEntry.value,
		);
		...
		switch (dataType) {
			case MongoBSONTypes.Object: {
				...
				Object.entries(stackEntry.value as ObjectId).map(
					([key, value]) => {
						stack.push({
							key: `${key}`,
							value: value,
			...
			default: {
				...
				tree.push({
					...
					type: MongoBSONTypes.toDisplayString(
						MongoBSONTypes.inferType(stackEntry.value),
					),
					parentId: stackEntry.parentId,
				});
