...
export function updateSchemaWithDocument(
	schema: JSONSchema,
	...
	for (const [name, value] of Object.entries(document)) {
		...
		propertySchema["x-occurrence"] =
			(propertySchema["x-occurrence"] ?? 0) + 1;

		// Find or create the type entry in 'anyOf'
		let typeEntry = findTypeEntry(
			propertySchema.anyOf as JSONSchema[],
			mongoDatatype,
		);
		...
		typeEntry["x-typeOccurrence"] =
			(typeEntry["x-typeOccurrence"] ?? 0) + 1;
		...
		switch (item.fieldMongoType) {
			...
			case MongoBSONTypes.Array: {
				...
				for (const element of arrayValue) {
					...
					let itemEntry = findTypeEntry(
						itemsSchema.anyOf as JSONSchema[],
						elementMongoType,
					);
					...
					// Increment the type occurrence count
					itemEntry["x-typeOccurrence"] =
						(itemEntry["x-typeOccurrence"] ?? 0) + 1;
					...
					if (
						elementMongoType === MongoBSONTypes.Object ||
						elementMongoType === MongoBSONTypes.Array
					) {
						fifoQueue.push({
		...
		switch (item.fieldMongoType) {
			case MongoBSONTypes.Object: {
				...
				for (const [name, value] of Object.entries(
					...
					item.propertyTypeEntry.properties[name] = {
						anyOf: [typeEntry],
						"x-occurrence": 1,
					};
			...
			case MongoBSONTypes.Array: {
				...
				for (const element of item.fieldValue as unknown[]) {
					...
					if (!encounteredMongoTypes.has(elementMongoType)) {
						...
						itemEntry = encounteredMongoTypes.get(
							elementMongoType,
						) as JSONSchema;
 ...
 */
function initializeStatsForValue(
	value: unknown,
	mongoType: MongoBSONTypes,
	propertyTypeEntry: JSONSchema,
): void {
	switch (mongoType) {
 ...
 */
function aggregateStatsForValue(
	value: unknown,
	mongoType: MongoBSONTypes,
	propertyTypeEntry: JSONSchema,
): void {
	switch (mongoType) {
		case MongoBSONTypes.String: {
			...
			// Update minLength
			if (
				propertyTypeEntry["x-minLength"] === undefined ||
				currentLength < propertyTypeEntry["x-minLength"]
			) {
				propertyTypeEntry["x-minLength"] = currentLength;
			}
			...
			// Update maxLength
			if (
				propertyTypeEntry["x-maxLength"] === undefined ||
				currentLength > propertyTypeEntry["x-maxLength"]
			) {
				propertyTypeEntry["x-maxLength"] = currentLength;
			}
		...
		case MongoBSONTypes.Decimal128: {
			...
			// Update minValue
			if (
				propertyTypeEntry["x-minValue"] === undefined ||
				numericValue < propertyTypeEntry["x-minValue"]
			) {
				propertyTypeEntry["x-minValue"] = numericValue;
			}
			...
			// Update maxValue
			if (
				propertyTypeEntry["x-maxValue"] === undefined ||
				numericValue > propertyTypeEntry["x-maxValue"]
			) {
				propertyTypeEntry["x-maxValue"] = numericValue;
			}
		...
		case MongoBSONTypes.Date: {
			...
			// Update minDate
			if (
				propertyTypeEntry["x-minDate"] === undefined ||
				dateValue < propertyTypeEntry["x-minDate"]
			) {
				propertyTypeEntry["x-minDate"] = dateValue;
			}
			...
			// Update maxDate
			if (
				propertyTypeEntry["x-maxDate"] === undefined ||
				dateValue > propertyTypeEntry["x-maxDate"]
			) {
				propertyTypeEntry["x-maxDate"] = dateValue;
			}
		...
		case MongoBSONTypes.Binary: {
			...
			if (
				propertyTypeEntry["x-minLength"] === undefined ||
				binaryLength < propertyTypeEntry["x-minLength"]
			) {
				propertyTypeEntry["x-minLength"] = binaryLength;
			}
			...
			if (
				propertyTypeEntry["x-maxLength"] === undefined ||
				binaryLength > propertyTypeEntry["x-maxLength"]
			) {
				propertyTypeEntry["x-maxLength"] = binaryLength;
			}
...
function getSchemaAtPath(schema: JSONSchema, path: string[]): JSONSchema {
	...
	for (let i = 0; i < path.length; i++) {
		...
		if (
			...
			const nextNode: JSONSchema = currentNode.properties[
				key
			] as JSONSchema;
			/**
			...
			if (nextNode.anyOf && nextNode.anyOf.length > 0) {
				currentNode = nextNode.anyOf.find(
					(entry: JSONSchema) => entry.type === "object",
				) as JSONSchema;
			} else {
			...
			throw new Error(
				`No properties found in the schema at path "${path.slice(0, i + 1).join("/")}"`,
			);
		}
	}
...
export function getPropertyNamesAtLevel(
