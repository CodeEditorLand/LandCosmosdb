...
import {
	getKnownFields,
	type FieldEntry,
} from "../../../utils/json/mongo/autocomplete/getKnownFields";
...
export const collectionsViewRouter = router({
	...
	runQuery: publicProcedure
		...
		.query(async ({ input, ctx }) => {
			...
			// run query
			const session: MongoClustersSession =
				MongoClustersSession.getSession(myCtx.sessionId);
	...
	getAutocompletionSchema: publicProcedure
		...
		.query(({ ctx }) => {
			...
			const autoCompletionData: FieldEntry[] =
				getKnownFields(_currentJsonSchema);
	...
	deleteDocumentsById: publicProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const session: MongoClustersSession =
				MongoClustersSession.getSession(myCtx.sessionId);
			const acknowledged = await session.deleteDocuments(
				myCtx.databaseName,
				myCtx.collectionName,
				input,
			);
			...
			if (acknowledged) {
				...
				void vscode.window.showErrorMessage(
					"Failed to delete documents. Unknown error.",
					{
						modal: true,
					},
				);
