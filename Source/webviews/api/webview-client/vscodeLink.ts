...
interface VSCodeLinkOptions {
	...
	onReceive: (
		onMessage: (message: VsCodeLinkResponseMessage) => void,
	...
	return (_runtime) => {
		...
		return ({ op }) => {
			...
			return observable((observer) => {
				const operationId =
					(op.context as { trpc?: { requestId?: string } })?.trpc
						?.requestId ?? Math.random().toString(16).substring(2);
