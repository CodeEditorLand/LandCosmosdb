		...
		async (activateContext: IActionContext) => {
			...
			context.subscriptions.push(
				vscode.workspace.registerFileSystemProvider(
					DatabasesFileSystem.scheme,
					ext.fileSystem,
				),
			);

			registerCommandWithTreeNodeUnwrapping(
				"cosmosDB.selectSubscriptions",
				() =>
					...
					if (platform() !== "win32") {
						...
						throw new Error(
							localize(
								...
								"The Cosmos DB emulator is only supported on Windows.",
							),
						);
					}
			...
			registerCommandWithTreeNodeUnwrapping(
				...
				async (
					actionContext: IActionContext & ITreeItemPickerContext,
					node?: AzExtTreeItem,
				) => {
					const children =
						await ext.attachedAccountsNode.loadAllChildren(
							actionContext,
						);
					if (
						children[0].contextValue ===
						"cosmosDBAttachDatabaseAccount"
					) {
						...
						if (!node) {
							node =
								await ext.rgApi.workspaceResourceTree.showTreeItemPicker<AzExtTreeItem>(
									cosmosDBTopLevelContextValues.map(
										(val: string) =>
											(val += AttachedAccountSuffix),
									),
									actionContext,
								);
						}
			...
			registerCommandWithTreeNodeUnwrapping(
				...
				async (
					actionContext: IActionContext,
					node?: MongoDocumentTreeItem | DocDBDocumentTreeItem,
				) => {
					if (!node) {
						node = await ext.rgApi.pickAppResource<
							MongoDocumentTreeItem | DocDBDocumentTreeItem
						>(actionContext, {
			...
			registerCommandWithTreeNodeUnwrapping(
				...
				async (actionContext: IActionContext, node: AzExtTreeItem) =>
					await ext.rgApi.appResourceTree.loadMore(
						node,
						actionContext,
					),
			);
				...
				async (
					...
					if (
						event.affectsConfiguration(
							ext.settingsKeys.documentLabelFields,
						)
					) {
...
export async function createServer(
	...
	if (!node) {
		node =
			await ext.rgApi.appResourceTree.showTreeItemPicker<SubscriptionTreeItem>(
