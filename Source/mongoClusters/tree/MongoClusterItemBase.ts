...
export abstract class MongoClusterItemBase implements TreeElementBase {
	...
	async getChildren(): Promise<TreeElementBase[]> {
		...
		return mongoClustersClient
			.listDatabases()
			.then((databases: DatabaseItemModel[]) => {
				...
				return databases.map(
					(database) => new DatabaseItem(this.mongoCluster, database),
				);
			});
	}
	...
	async createDatabase(
		...
		const client = await MongoClustersClient.getClient(
			this.mongoCluster.id,
		);
	...
	getTreeItem(): TreeItem {
		return {
			...
			description:
				this.mongoCluster.sku !== undefined
					? `(${this.mongoCluster.sku})`
					: false,
			...
			tooltip: new vscode.MarkdownString(
				`### Cluster: ${this.mongoCluster.name}\n\n` +
					...
					(this.mongoCluster.location
						...
						: "") +
					(this.mongoCluster.diskSize
						? `- Disk Size: **${this.mongoCluster.diskSize}GB**\n`
						: "") +
					(this.mongoCluster.sku
						? `- SKU: **${this.mongoCluster.sku}**\n`
						: "") +
					(this.mongoCluster.enableHa !== undefined
						? `- High Availability: **${this.mongoCluster.enableHa ? "Enabled" : "Disabled"}**\n`
						: "") +
					(this.mongoCluster.nodeCount
						? `- Node Count: **${this.mongoCluster.nodeCount}**\n\n`
						: "") +
