			...
			async (context: IActionContext) => {
				...
				if (this.detailsCacheUpdateRequested) {
					void this.updateResourceCache(
						 ...
						 */
						this.itemsToUpdateInfo.forEach(
							(value: MongoClusterResourceItem) => {
								value.mongoCluster = {
									...value.mongoCluster,
									...this.detailsCache.get(
				...
				const clusterItem = new MongoClusterResourceItem(
					element.subscription,
					clusterInfo,
				);
		...
		return callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				try {
					...
					const client = await createMongoClustersManagementClient(
						_context,
						subscription,
					);

					const accounts = await uiUtils.listAllIterator(
						client.mongoClusters.list(),
					);

					accounts.map((MongoClustersAccount) => {
						// eslint-disable-next-line @typescript-eslint/no-unsafe-argument
						this.detailsCache.set(
							nonNullProp(MongoClustersAccount, "id"),
							{
								id: MongoClustersAccount.id as string,
								name: MongoClustersAccount.name as string,
								resourceGroup: getResourceGroupFromId(
									MongoClustersAccount.id as string,
								),

								location:
									MongoClustersAccount.location as string,
								serverVersion:
									MongoClustersAccount.serverVersion as string,

								systemData: {
									createdAt:
										MongoClustersAccount.systemData
											?.createdAt,
								},

								sku:
									MongoClustersAccount.nodeGroupSpecs !==
									undefined
										? (MongoClustersAccount
												.nodeGroupSpecs[0]
												?.sku as string)
										: undefined,
								diskSize:
