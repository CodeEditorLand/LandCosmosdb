...
export class MongoClusterResourceItem extends MongoClusterItemBase {
		...
		const result = await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				...
				const managementClient =
					await createMongoClustersManagementClient(
						context,
						this.subscription,
					);

				const clusterInformation =
					await managementClient.mongoClusters.get(
						this.mongoCluster.resourceGroup as string,
						this.mongoCluster.name,
					);

				const clusterConnectionString = nonNullValue(
					clusterInformation.connectionString,
				);

				context.valuesToMask.push(clusterConnectionString);

				// Fetch non-admin users using the extracted method
				const clusterNonAdminUsers =
					await this.fetchNonAdminUsersFromAzure(
						managementClient,
						clusterInformation,
					);
				...
				const credentialsProvided =
					await this.promptForCredentials(wizardContext);
	...
	private async promptForCredentials(
		wizardContext: AuthenticateWizardContext,
	): Promise<boolean> {
	...
	private async fetchNonAdminUsersFromAzure(
		...
		const clusterNonAdminUsers: string[] =
			await listMongoClusterNonAdminUsers(client, {
		...
		ext.outputChannel.appendLine(
			`Discovered ${clusterNonAdminUsers.length} non-admin user(s).`,
		);
