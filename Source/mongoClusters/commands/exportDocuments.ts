...
export async function mongoClustersExportEntireCollection(
...
export async function mongoClustersExportQueryResults(
	...
	ext.outputChannel.appendLog(
		`MongoDB Clusters: Exporting data to: ${filePath}`,
	);
	...
	await callWithTelemetryAndErrorHandling(
		"cosmosDB.mongoClusters.exportDocuments",
		async (actionContext) => {
			await runExportWithProgressAndDescription(
				node.id,
	...
	ext.outputChannel.appendLog(
		`MongoDB Clusters: Exported document count: ${documentCount}`,
	);
}
	...
	try {
		...
		for await (const doc of documentStream) {
			...
			if (documentCount % 100 === 0) {
				progress.report({
					message: `${documentCount} documents exported...`,
				});
			}
		...
		vscode.window.showInformationMessage(
			`Exported document count: ${documentCount}`,
		);
	} catch (error) {
...
async function askForTargetFile(
