 ...
 *
 */
export async function getMongoClusterMetadata(
	client: MongoClient,
): Promise<MongoClusterMetadata> {
	...
	try {
		...
		result["serverInfo.storageEngines"] = (
			buildInfo.storageEngines as string[]
		)?.join(";");
	...
	try {
		...
		result["serverStatus.connections.current"] =
			serverStatus.connections?.current.toString();
		result["serverStatus.connections.available"] =
			serverStatus.connections?.available.toString();
		result["serverStatus.memory.resident"] =
			serverStatus.mem?.resident.toString();
		result["serverStatus.memory.virtual"] =
			serverStatus.mem?.virtual.toString();
	} catch (error) {
	...
	try {
		...
		result["topology.numberOfServers"] = (
			helloInfo.hosts?.length || 0
		).toString();
