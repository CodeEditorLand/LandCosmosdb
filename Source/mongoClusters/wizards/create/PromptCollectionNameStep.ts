...
export class CollectionNameStep extends AzureWizardPromptStep<CreateCollectionWizardContext> {
	...
	public static validateInput(
		...
		if (/[$]/.test(collectionName)) {
			return localize(
				"mongoClusters.collectionNameContainsDollar",
				"Collection name cannot contain the $.",
			);
		}
		...
		if (collectionName.includes(".system.")) {
			return localize(
				"mongoClusters.collectionNameContainsSystem",
				"Collection name cannot contain .system.",
			);
		}
	...
	private async validateNameAvailable(
		...
		if (name.length === 0) {
			return localize(
				"mongoClusters.collectionNameRequired",
