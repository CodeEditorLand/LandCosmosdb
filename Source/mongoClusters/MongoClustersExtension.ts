	...
	async activate(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			...
			async (activateContext: IActionContext) => {
				...
				const isMongoClustersEnabled: boolean =
					isMongoClustersSupportenabled() ?? false;

				activateContext.telemetry.properties.mongoClustersEnabled =
					isMongoClustersEnabled.toString();
