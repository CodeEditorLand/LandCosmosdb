...
} from "../utils/json/mongo/SchemaAnalyzer";
import { getDataAtPath } from "../utils/slickgrid/mongo/toSlickGridTable";
import {
	toSlickGridTree,
	type TreeData,
} from "../utils/slickgrid/mongo/toSlickGridTree";
...
export class MongoClustersSession {
	...
	private resetCachesIfQueryChanged(query: string) {
		if (
			this._currentQueryText.localeCompare(query.trim(), undefined, {
				sensitivity: "base",
			}) === 0
		) {
			return;
	...
	public async runQueryWithCache(
		...
		// JSON Schema
		this._currentRawDocuments.map((doc) =>
			updateSchemaWithDocument(this._currentJsonSchema, doc),
		);
	...
	public getCurrentPageAsJson(): string[] {
		return this._currentRawDocuments.map((doc) =>
	...
	async deleteDocuments(
		...
		const acknowledged = await this._client.deleteDocuments(
			databaseName,
			collectionName,
			documentIds,
		);
		...
		if (acknowledged) {
			this._currentRawDocuments = this._currentRawDocuments.filter(
				(doc) => {
					...
					return !documentIds.some((id) => {
						...
						const docIdStr = EJSON.stringify(
							doc._id,
							{ relaxed: false },
							0,
						);
						const parsedIdStr = EJSON.stringify(
							parsedId,
							{ relaxed: false },
							0,
						);
