...
import {
	getMongoClusterMetadata,
	type MongoClusterMetadata,
} from "./utils/getMongoClusterMetadata";
...
export class MongoClustersClient {
	...
	private async initClient(credentialId: string): Promise<void> {
		...
		const cString = CredentialCache.getCredentials(credentialId)
			?.connectionString as string;
		const hosts = getHostsFromConnectionString(cString);
		const userAgentString = areMongoDBAzure(hosts)
			? appendExtensionUserAgent()
			: undefined;

		const cStringPassword =
			CredentialCache.getConnectionStringWithPassword(credentialId);

		this._mongoClient = await MongoClient.connect(
			cStringPassword as string,
			{
		...
		void callWithTelemetryAndErrorHandling(
			"cosmosDB.mongoClusters.connect.getmetadata",
			async (context) => {
				const metadata: MongoClusterMetadata =
					await getMongoClusterMetadata(this._mongoClient);
	...
	async listCollections(
		databaseName: string,
	): Promise<CollectionItemModel[]> {
		const rawCollections = await this._mongoClient
			.db(databaseName)
			.listCollections()
			.toArray();
	...
	async listIndexes(
		...
		const collection = this._mongoClient
			.db(databaseName)
		...
		try {
			while (await cursor.hasNext()) {
				if (abortSignal.aborted) {
					console.debug(
						"streamDocuments: Aborted by an abort signal.",
					);
					return;
				}
	...
	async pointRead(
		...
		// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
		const documentContent = await collection.findOne({
