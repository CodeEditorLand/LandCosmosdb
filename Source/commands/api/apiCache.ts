...
const sessionCache: Map<string, DatabaseAccountTreeItem | DatabaseTreeItem> =
	new Map();
...
export function removeTreeItemFromCache(
	...
	if (!expected.databaseName) {
		...
		for (const [key, value] of sessionCache.entries()) {
			...
			if (expected instanceof ParsedPostgresConnectionString) {
				...
				actual = new ParsedMongoConnectionString(
				...
				actual = new ParsedDocDBConnectionString(
					value.connectionString,
					value.hostName,
					value.port,
					undefined,
