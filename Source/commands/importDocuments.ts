...
export async function importDocuments(
	...
	if (ignoredUris.length) {
		ext.outputChannel.appendLog(
			`Ignoring the following files which are not json:`
		);
		ignoredUris.forEach((uri) =>
			ext.outputChannel.appendLine(`${uri.fsPath}`)
		);
	...
	result = await vscode.window.withProgress(
		...
		async (progress) => {
			...
			progress.report({
				increment: 20,
				message: "Parsing documents for errors",
			});
			const documents = await parseDocuments(uris);
			progress.report({
				increment: 30,
				message: "Parsed documents. Importing",
			});
...
async function insertDocumentsIntoDocdb(
	...
	if (erroneousFiles.length) {
		...
		erroneousFiles.forEach((file) =>
			ext.outputChannel.appendLine(file.path)
		);
		ext.outputChannel.show();
		throw new Error(
			`See output for list of documents that do not contain the partition key '${
				nonNullProp(collectionNode, "partitionKey").paths[0]
			}' required by collection '${collectionNode.label}'`
		);
	}
