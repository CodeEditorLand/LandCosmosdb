...
export async function importDocuments(
	...
	result = await vscode.window.withProgress(
		...
		async (progress) => {
			...
			const supportEJSON: boolean =
				collectionNode instanceof CollectionItem; // added this line for better readability
			const documents: unknown[] = await parseDocuments(
				uris,
				supportEJSON,
			);
			...
			progress.report({
				increment: 30,
				message: `Loaded ${documents.length} document(s). Importing...`,
			});
			if (collectionNode instanceof MongoCollectionTreeItem) {
				result = await insertDocumentsIntoMongo(
					collectionNode,
					documents,
				);
			} else if (collectionNode instanceof CollectionItem) {
				result = await insertDocumentsIntoMongoCluster(
					context,
					collectionNode,
					documents,
				);
			} else {
				result = await insertDocumentsIntoDocdb(
					collectionNode,
					documents,
					uris,
				);
...
async function insertDocumentsIntoDocdb(
	...
	for (i = 0; i < documents.length; i++) {
		...
		if (
			!collectionNode.documentsTreeItem.documentHasPartitionKey(document)
		) {
	...
	for (const document of documents) {
		...
		const retrieved: ItemDefinition =
			await collectionNode.documentsTreeItem.createDocument(document);
		if (retrieved.id) {
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function insertDocumentsIntoMongo(
	node: MongoCollectionTreeItem,
	documents: any[],
): Promise<string> {
	let output = "";

	const parsed = await callWithTelemetryAndErrorHandling(
		"cosmosDB.mongo.importDocumets",
		async (actionContext) => {
			// eslint-disable-next-line @typescript-eslint/no-unsafe-argument
			const parsed = await node.collection.insertMany(documents);

			actionContext.telemetry.measurements.documentCount =
...
async function insertDocumentsIntoMongoCluster(
	...
	const result = await callWithTelemetryAndErrorHandling(
		...
		async (actionContext) => {
			const result = await node.insertDocuments(
				context,
