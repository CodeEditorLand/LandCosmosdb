...
export class DocumentTab extends BaseTab {
	...
	protected constructor(
		...
		super(panel, DocumentTab.viewType, {
	...
	public static render(
		...
		if (documentId) {
			const openTab = [...DocumentTab.openTabs].find((openTab) => {
				...
				if (
					documentId._rid &&
					openTab.documentId._rid &&
					openTab.documentId._rid === documentId._rid
				) {
					return true;
				}

				if (
					documentId.partitionKey !== undefined &&
					openTab.documentId.partitionKey !== undefined
				) {
					const openTabPK = Array.isArray(
						openTab.documentId.partitionKey,
					)
					...
					return (
						documentId.id === openTab.documentId.id &&
						openTabPK === pk
					);
				}
	...
	private async refreshDocument(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.document.refreshDocument",
			async () => {
				if (this.documentId) {
					await this.session.read(this.documentId);
				} else {
	...
	private async saveDocument(documentText: string): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			...
			async (context) => {
				const documentContent: JSONValue = JSON.parse(
					documentText,
				) as JSONValue;

				if (!this.isCosmosDbItemDefinition(documentContent)) {
					throw new Error(
						"Document is not a valid Cosmos DB item definition",
					);
				}
	...
	private isCosmosDbItemDefinition(
		documentContent: unknown,
	): documentContent is ItemDefinition {
		if (
			documentContent &&
