...
export class QueryEditorTab extends BaseTab {
	...
	protected constructor(
		...
		super(panel, QueryEditorTab.viewType, {
		...
		switch (commandName) {
			...
			case "runQuery":
				return this.runQuery(
					payload.params[0] as string,
					payload.params[1] as ResultViewMetadata,
				);
			...
			case "openDocument":
				return this.openDocument(
					payload.params[0] as string,
					payload.params[1] as CosmosDbRecordIdentifier,
				);

			case "deleteDocument":
				return this.deleteDocument(
					payload.params[0] as CosmosDbRecordIdentifier,
				);
		}
	...
	private async updateConnection(
		...
		if (this.connection) {
			const { databaseId, containerId, endpoint, masterKey } =
				this.connection;

			this.telemetryContext.addMaskedValue([
				databaseId,
				containerId,
				endpoint,
				masterKey ?? "",
			...
			const params: (PartitionKeyDefinition | string)[] = [
				databaseId,
				containerId,
			];
	...
	private async openFile(): Promise<void> {
		...
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.openFile",
			async () => {
				await vscode.window.showOpenDialog(options).then((fileUri) => {
					if (fileUri && fileUri[0]) {
						return vscode.workspace
							.openTextDocument(fileUri[0])
							.then((document) => {
	...
	private async saveFile(
		...
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.saveFile",
			async () => {
				if (!ext.startsWith(".")) {
					ext = `.${ext}`;
				}
	...
	private async duplicateTab(text: string): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.duplicateTab",
			() => {
	...
	private async copyToClipboard(text: string): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.copyToClipboard",
			async () => {
	...
	private async connectToDatabase(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.connectToDatabase",
			async (context) => {
	...
	private async disconnectFromDatabase(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.disconnectFromDatabase",
			async () => {
	...
	private async stopQuery(executionId: string): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.stopQuery",
			async (context) => {
				...
				if (!session) {
					throw new Error(
						`No session found for executionId: ${executionId}`,
					);
				}
		...
		await callWithTelemetryAndErrorHandling(
			...
			() => {
				...
				if (!documentId && mode !== "add") {
					throw new Error(
						"Impossible to open a document without an id",
					);
				}
