...
export class CommonChannel implements Channel {
	...
	postMessage(
		...
		return (
			this.pendingRequests[id]?.deferred.promise ?? Promise.resolve(true)
		);
	}
		...
		try {
				...
				callbacks.forEach((cb) => {
					...
					try {
						...
						void Promise.resolve(
							cb.callback.call(undefined, ...payload.params),
						)
							.then((returnValue) => {
								if (payload.type === "request") {
			...
			if (this.pendingRequests[msg.id]) {
				this.pendingRequests[msg.id].deferred.reject(
					new Error(
						`Error occurred handling received message : ${errorMessage}`,
					),
				);
			}
