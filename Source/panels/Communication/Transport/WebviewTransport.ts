	...
	private handleMessage(msg: unknown): void {
		try {
			if (!isTransportMessage(msg)) {
				console.warn(
					`[WebviewTransport] Received message that is not a transport message`,
					msg,
				);
			...
			this.listeners.forEach((cb) => {
				...
				try {
					...
					console.error(
						`[WebviewTransport] Error occurred calling callback`,
						error,
					);
				}
			});
			...
			console.error(
				`[WebviewTransport] Error occurred handling received message`,
				error,
			);
		}
	}
