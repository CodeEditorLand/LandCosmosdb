...
export class SharedWorkspaceStorage {
	private static readonly storageName: string =
		"ms-azuretools.vscode-cosmosdb.workspace";
	...
	public static async getItems(
		...
		for (const key of keys) {
			const item =
				ext.context.globalState.get<SharedWorkspaceStorageItem>(key);
	...
	public static async push(
		...
		const existingItem =
			ext.context.globalState.get<SharedWorkspaceStorageItem>(storageKey);

		if (existingItem && !overwrite) {
			throw new Error(
				`An item with id '${item.id}' already exists for workspaceType '${workspaceType}'.`,
			);
		}
		...
		if (item.secrets && item.secrets.length > 0) {
			...
			try {
				...
				console.error(
					`Failed to store secrets for key ${secretKey}:`,
					error,
				);
