...
export class AttachedAccountsTreeItem extends AzExtParentTreeItem {
	public static contextValue: string =
		"cosmosDBAttachedAccounts" +
		(isWindows ? "WithEmulator" : "WithoutEmulator");
	public readonly contextValue: string =
		AttachedAccountsTreeItem.contextValue;
	public readonly label: string = "Attached Database Accounts";
	public childTypeLabel: string = "Account";
	public suppressMaskLabel = true;

	private readonly _serviceName: string =
		"ms-azuretools.vscode-cosmosdb.connectionStrings";
	...
	public static validateMongoConnectionString(
		value: string
	): string | undefined {
	...
	public static validatePostgresConnectionString(
		value: string
	): string | undefined {
		value = value ? value.trim() : "";
	...
	private static validateDocDBConnectionString(
		value: string
	): string | undefined {
		value = value ? value.trim() : "";
	...
	public async loadMoreChildrenImpl(
		...
		if (attachedAccounts.length > 0) {
			...
			return isWindows
				? [attachDatabaseAccount, attachEmulator]
	...
	public async attachNewAccount(context: IActionContext): Promise<void> {
		...
		if (defaultExperience.api === API.MongoDB) {
			...
			validateInput =
				AttachedAccountsTreeItem.validateMongoConnectionString;
		} else if (
			defaultExperience.api === API.PostgresSingle ||
			...
			validateInput =
				AttachedAccountsTreeItem.validatePostgresConnectionString;
		} else {
			placeholder = "AccountEndpoint=...;AccountKey=...";
			validateInput =
				AttachedAccountsTreeItem.validateDocDBConnectionString;
		}
		...
		const treeItem: AzExtTreeItem = await this.createTreeItem(
			connectionString,
			defaultExperience.api
		);
	...
	public async attachEmulator(context: IActionContext): Promise<void> {
		...
		if (defaultExperience.api === API.MongoDB) {
			port = vscode.workspace
				.getConfiguration()
				.get<number>("cosmosDB.emulator.mongoPort");
		} else {
			port = vscode.workspace
				.getConfiguration()
				.get<number>("cosmosDB.emulator.port");
		}
		if (port) {
			if (defaultExperience.api === API.MongoDB) {
				...
				connectionString = `mongodb://localhost:${encodeURIComponent(
					emulatorPassword
				)}@localhost:${port}/?ssl=true`;
			} else {
	...
	public async detach(node: AzExtTreeItem): Promise<void> {
		...
		const index = attachedAccounts.findIndex(
			(account) => account.fullId === node.fullId
		);
		if (index !== -1) {
			attachedAccounts.splice(index, 1);
			await ext.secretStorage.delete(
				getSecretStorageKey(this._serviceName, nonNullProp(node, "id"))
			...
			if (node instanceof MongoAccountTreeItem) {
				const parsedCS = await parseMongoConnectionString(
	...
	private async getAttachedAccounts(): Promise<AzExtTreeItem[]> {
		if (!this._attachedAccounts) {
			try {
				...
				throw new Error(
					"Failed to load persisted Database Accounts. Reattach the accounts manually."
				);
			}
		}
	...
	private async attachAccount(
		...
		if (attachedAccounts.find((s) => s.id === treeItem.id)) {
			void context.ui.showWarningMessage(
				`Database Account '${treeItem.id}' is already attached.`,
			...
			await Promise.all(
				accounts.map(async (account) => {
					...
					if (typeof account === "string") {
						...
						api = API.MongoDB;
						label = `${account} (${
							getExperienceFromApi(api).shortName
						})`;
						isEmulator = false;
					} else {
					...
					const connectionString: string = nonNullValue(
						await ext.secretStorage.get(
							getSecretStorageKey(this._serviceName, id)
						),
						"connectionString"
					);
	...
	private async createTreeItem(
		...
		if (api === API.MongoDB) {
			...
			treeItem = new MongoAccountTreeItem(
				this,
				id,
				label,
				connectionString,
			...
			treeItem = new PostgresServerTreeItem(
				this,
				parsedPostgresConnString
			);
		} else {
	...
	private async persistIds(attachedAccounts: AzExtTreeItem[]): Promise<void> {
		const value: IPersistedAccount[] = attachedAccounts.map(
			(node: AzExtTreeItem) => {
				let api: API;
				let isEmulator: boolean | undefined;
				if (
				...
				if (node instanceof MongoAccountTreeItem) {
					...
					throw new Error(
						`Unexpected account node "${node.constructor.name}".`
					);
				}
				return {
					id: nonNullProp(node, "id"),
					defaultExperience: api,
