...
import {
	type AbstractFirewallRule,
	type PostgresServerType,
...
export async function configurePostgresFirewall(
	...
	if (!treeItem) {
		treeItem = await ext.rgApi.pickAppResource<PostgresServerTreeItem>(
			context,
			{
...
export async function setFirewallRule(
	...
	const firewallRuleName: string =
		"azDbVSCode-Ip" + `-${randomUtils.getRandomHexString(6)}`;
