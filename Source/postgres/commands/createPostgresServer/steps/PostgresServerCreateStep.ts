...
export class PostgresServerCreateStep extends AzureWizardExecuteStep<IPostgresServerWizardContext> {
		...
		return await callWithMaskHandling(async () => {
			...
			switch (serverType) {
				case PostgresServerType.Single:
					const singleClient: SingleModels.PostgreSQLManagementClient =
						await createPostgreSQLClient(context);
					context.server =
						await singleClient.servers.beginCreateAndWait(
							rgName,
							newServerName,
				...
				case PostgresServerType.Flexible:
					const flexiClient: FlexibleModels.PostgreSQLManagementClient =
						await createPostgreSQLFlexibleClient(context);
					context.server =
						await flexiClient.servers.beginCreateAndWait(
							rgName,
							newServerName,
	...
	private asFlexibleParameters(
		parameters: AbstractServerCreate
	): FlexibleModels.Server {
		return {
	...
	private asSingleParameters(
		parameters: AbstractServerCreate
	): SingleModels.ServerForCreate {
		return {
			location: parameters.location,
			sku: {
				name: parameters.sku.name,
