...
export class PostgresServerNameStep extends AzureNameStep<IPostgresServerWizardContext> {
	public async prompt(context: IPostgresServerWizardContext): Promise<void> {
		const client = await createAbstractPostgresClient(
		...
		context.newServerName = (
			await context.ui.showInputBox({
				...
				prompt: localize(
					"enterServerNamePrompt",
					"Provide a name for the PostgreSQL Server.",
				),
...
async function validatePostgresServerName(
	...
	if (!availability.nameAvailable) {
		return availability.message
			...
			: localize(
					"serverNameAvailabilityCheck",
					'Server name "{0}" is not available.',
					name,
				);
	}
