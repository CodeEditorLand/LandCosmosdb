...
export async function executePostgresQueryInDocument(
	...
	const clientConfig: ClientConfig = await checkAuthentication(
		context,
		treeItem,
	);
	...
	if (!activeEditor?.document) {
		throw new Error(
			localize(
				"openQueryBeforeExecuting",
				"Open a PostgreSQL query before executing.",
			),
		);
	}
