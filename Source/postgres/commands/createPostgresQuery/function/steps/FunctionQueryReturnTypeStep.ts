...
export class FunctionQueryReturnTypeStep extends AzureWizardPromptStep<IPostgresFunctionQueryWizardContext> {
	public async prompt(
		...
		const returnTypeQuickPicks: IAzureQuickPickItem<string | undefined>[] =
			returnTypes.map((r) => {
		...
		returnTypeQuickPicks.push({
			label: localize(
				...
				"$(pencil) Enter custom return type...",
			),
			data: undefined,
		});
	...
	public async getSubWizard(
		...
		IWizardOptions<IPostgresFunctionQueryWizardContext> | undefined
	> {
		return context.returnType
			? undefined
			: { promptSteps: [new FunctionQueryCustomReturnTypeStep()] };
	}
}
