...
export class PostgresServerTreeItem extends AzExtParentTreeItem {
	...
	public get label(): string {
		return this.azureName
			? this.azureName
			: this.partialConnectionString.fullId;
	}
	...
	public async loadMoreChildrenImpl(
		...
		if (this.azureName) {
			const client: AbstractPostgresClient =
				await createAbstractPostgresClient(this.serverType, [
					context,
					this.subscription,
				]);

			const listOfDatabases: (
				| SingleModels.Database
				| FlexibleModels.Database
			)[] = await uiUtils.listAllIterator(
				client.databases.listByServer(
	...
	public async createChildImpl(
	...
	): Promise<PostgresDatabaseTreeItem> {
		...
		const databaseName = await context.ui.showInputBox({
			...
			validateInput: (name: string) =>
				validateDatabaseName(name, getChildrenTask),
		});
	...
	public async deleteTreeItemImpl(context: IActionContext): Promise<void> {
		const client = await createAbstractPostgresClient(this.serverType, [
			context,
			this.subscription,
		]);
	...
	public async supportsStoredProcedures(
	...
	public async deletePostgresCredentials(): Promise<void> {
		...
		let servers: IPersistedServer[] = storedValue
	...
	public async getFullConnectionString(): Promise<ParsedPostgresConnectionString> {
		if (
			this.azureId &&
			!(
				this.partialConnectionString.username &&
				this.partialConnectionString.password
			)
		) {
			const storedValue: string | undefined = ext.context.globalState.get(
				PostgresServerTreeItem.serviceName,
			);
	...
	public async isFirewallRuleSet(context: IActionContext): Promise<boolean> {
		try {
			const serverType: PostgresServerType = nonNullProp(
				this,
				"serverType",
			);

			const client: AbstractPostgresClient =
				await createAbstractPostgresClient(serverType, [
					context,
					this.subscription,
				]);

			const results: SingleModels.FirewallRule[] =
				await uiUtils.listAllIterator(
