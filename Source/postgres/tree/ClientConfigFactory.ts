...
import {
	getAzureAdUserSession,
	getTokenFunction,
} from "../../azureAccountUtils";
import { localize } from "../../utils/localize";
import {
	PostgresClientConfigType,
	getClientConfigs,
	testClientConfig,
} from "../getClientConfig";
import {
	firewallNotConfiguredErrorType,
	invalidCredentialsErrorType,
	timeoutErrorType,
} from "../postgresConstants";
import { PostgresServerTreeItem } from "./PostgresServerTreeItem";

export const postgresResourceType =
...
export class PostgresClientConfigFactory {
	public static async getClientConfigFromNode(
		...
		const clientConfigTypeOrder: PostgresClientConfigType[] = [
			"azureAd",
			"password",
			"connectionString",
		];
		...
		for (const clientConfigType of clientConfigTypeOrder) {
			...
			try {
				await callWithTelemetryAndErrorHandling<void>(
				...
				if (parsedError.errorType === invalidCredentialsErrorType) {
					...
					throw {
						message: localize(
							...
							"Must configure firewall from Azure Portal to grant access."
						),
		...
		throw {
			message: localize(
				"mustEnterCredentials",
				"Must enter credentials to connect to server."
			),
