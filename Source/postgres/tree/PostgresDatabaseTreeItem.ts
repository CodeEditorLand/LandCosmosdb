...
export class PostgresDatabaseTreeItem extends AzExtParentTreeItem {
	...
	public get description(): string {
		return ext.connectedPostgresDB?.fullId === this.fullId
	...
	public async loadMoreChildrenImpl(
		...
		if (!isFirewallRuleSet) {
			const firewallTreeItem: AzExtTreeItem = new GenericTreeItem(this, {
				...
				label: localize(
					"configureFirewall",
					'Configure firewall to connect to "{0}"...',
					this.parent.label,
				),
		...
		try {
			...
			if (await this.parent.supportsStoredProcedures(clientConfig)) {
				children.push(
					new PostgresStoredProceduresTreeItem(this, clientConfig),
				);
			}
			...
			if (
				this.parent.azureName &&
				parsedError.errorType === invalidCredentialsErrorType
			) {
				...
				const credentialsTreeItem: AzExtTreeItem = new GenericTreeItem(
					this,
					{
