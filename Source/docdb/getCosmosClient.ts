...
export function getCosmosKeyCredential(
	credentials: CosmosDBCredential[],
): CosmosDBKeyCredential | undefined {
	return credentials.filter(
		(cred): cred is CosmosDBKeyCredential => cred.type === "key",
	)[0];
}

export function getCosmosAuthCredential(
	credentials: CosmosDBCredential[],
): CosmosDBAuthCredential | undefined {
	return credentials.filter(
		(cred): cred is CosmosDBAuthCredential => cred.type === "auth",
	)[0];
}
...
export function getCosmosClientByConnection(
	...
	const connectionPolicy = {
		enableEndpointDiscovery:
			enableEndpointDiscovery === undefined
				? true
				: enableEndpointDiscovery,
	};
	...
	if (masterKey !== undefined) {
		...
		commonProperties.aadCredentials = {
			getToken: async (scopes, _options) => {
				const session = await getSessionFromVSCode(scopes, undefined, {
					createIfNone: true,
				});
