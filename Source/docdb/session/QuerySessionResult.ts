...
export class QuerySessionResult {
	...
	public push(response: FeedResponse<QueryResultRecord>): void {
		...
		this.queryResults.set(pageNumber, {
			...
			queryMetrics: (response.queryMetrics as unknown as QueryMetrics[])[
				"0"
			],
	...
	public getSerializedResult(
		pageNumber: number,
	): SerializedQueryResult | undefined {
		...
		if (result) {
			...
			if (result.queryMetrics) {
				serializedResult.queryMetrics = {
					documentLoadTime:
						result.queryMetrics.documentLoadTime.totalMilliseconds(),
					documentWriteTime:
						result.queryMetrics.documentWriteTime.totalMilliseconds(),
					indexHitDocumentCount:
						result.queryMetrics.indexHitDocumentCount,
					outputDocumentCount:
						result.queryMetrics.outputDocumentCount,
					outputDocumentSize: result.queryMetrics.outputDocumentSize,
					indexLookupTime:
						result.queryMetrics.indexLookupTime.totalMilliseconds(),
					retrievedDocumentCount:
						result.queryMetrics.retrievedDocumentCount,
					retrievedDocumentSize:
						result.queryMetrics.retrievedDocumentSize,
					vmExecutionTime:
						result.queryMetrics.vmExecutionTime.totalMilliseconds(),
					...
					totalQueryExecutionTime:
						result.queryMetrics.totalQueryExecutionTime.totalMilliseconds(),
				};
			}
