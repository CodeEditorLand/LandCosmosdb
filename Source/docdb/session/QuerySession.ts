...
export class QuerySession {
		...
		await callWithTelemetryAndErrorHandling(
			...
			async (context) => {
				...
				try {
					...
					const client = getCosmosClientByConnection(
						this.connection,
						{
							connectionPolicy: {
								requestTimeout:
									this.resultViewMetadata.timeout ??
									DEFAULT_EXECUTION_TIMEOUT,
							},
						},
					);
					...
					this.iterator = client
						...
						.items.query<QueryResultRecord>(this.query, {
							...
							maxItemCount: isFetchAll
								? undefined
								: (this.resultViewMetadata?.countPerPage ??
									DEFAULT_PAGE_SIZE),
	...
	public async fetchAll(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.session.fetchAll",
			async (context) => {
				this.setTelemetryProperties(context);
				...
				if (!this.iterator) {
					throw new Error(
						"Session is not running! Please run the session first",
					);
				}
	...
	public async nextPage(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			...
			async (context) => {
				...
				if (this.resultViewMetadata.countPerPage === -1) {
					throw new Error(
						"Cannot fetch next page if all records have been fetched before",
					);
				}

				if (
					this.sessionResult.getResult(this.currentIteration)
						?.hasMoreResults === false
				) {
					throw new Error(
						"Cannot fetch next page if current page is the last page",
					);
				}
	...
	public async prevPage(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			...
			async (context) => {
				...
				if (this.resultViewMetadata.countPerPage === -1) {
					throw new Error(
						"Cannot fetch previous page if all records have been fetched before",
					);
				}

				if (this.currentIteration - 1 <= 0) {
					throw new Error(
						"Cannot fetch previous page if current page is the first page",
					);
				}
	...
	public async stop(): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.queryEditor.session.stop",
			async (context) => {
				this.setTelemetryProperties(context);
	...
	private async errorHandling(
		...
		if (error instanceof ErrorResponse) {
			...
			(isObject && "name" in error && error.name === "AbortError")
		) {
			await this.channel.postMessage({
				type: "event",
				name: "queryError",
	...
	private async wrappedFetch(
		...
		try {
			...
			await this.channel.postMessage({
				...
				params: [
					this.id,
					this.sessionResult.getSerializedResult(
						this.currentIteration,
					),
					this.currentIteration,
				],
	...
	private async logAndThrowError(
		message: string,
		...
		if (error) {
			...
			if (
				...
				error.message.indexOf("ActivityId:") === 0
			) {
