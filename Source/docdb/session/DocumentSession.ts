...
export class DocumentSession {
	...
	public async create(
		...
		return callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.document.session.create",
			async (context) => {
					...
					if (response?.resource) {
						...
						return {
							...
							partitionKey: partitionKey
								? extractPartitionKey(record, partitionKey)
								: undefined,
						};
					} else {
						await this.channel.postMessage({
	...
	public async read(documentId: CosmosDbRecordIdentifier): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.document.session.read",
			async (context) => {
				this.setTelemetryProperties(context);
					...
					if (!result && documentId._rid) {
						const queryResult = await this.client
							...
							.items.query<CosmosDbRecord>(
								`SELECT * FROM c WHERE c._rid = "${documentId._rid}"`,
								{
									abortSignal: this.abortController.signal,
									bufferItems: true,
								},
	...
	public async update(
		...
		return callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.document.session.update",
			async (context) => {
				this.setTelemetryProperties(context);
		...
		await callWithTelemetryAndErrorHandling(
			...
			async (context) => {
				...
				partitionKey?.paths.forEach((partitionKeyProperty) => {
					...
					const keySegments = partitionKeyProperty
						.split("/")
					...
					target[finalSegment] =
						"replace_with_new_partition_key_value";
				});
	...
	private async getPartitionKey(): Promise<
	...
	> {
		...
		return callWithTelemetryAndErrorHandling(
			"cosmosDB.nosql.document.session.getPartitionKey",
			async () => {
