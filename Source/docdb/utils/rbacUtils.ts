...
export async function ensureRbacPermission(
	docDbItem: DocDBAccountTreeItemBase,
	principalId: string,
): Promise<boolean> {
	return (
			...
			async (context: IActionContext) => {
				...
				if (
					await askForRbacPermissions(
						accountName,
						docDbItem.subscription.subscriptionDisplayName,
						context,
					)
				) {
					...
					context.telemetry.measurements["createRoleAssignment"] =
						(Date.now() - start) / 1000;
...
export async function showRbacPermissionError(
	accountName: string,
	principalId: string,
): Promise<void> {
		...
		.then((item) => {
			if (item === readMoreItem) {
				void vscode.env.openExternal(
					vscode.Uri.parse("https://aka.ms/cosmos-native-rbac"),
				);
			}
		});
}
...
async function askForRbacPermissions(
	...
	const message = [
		...
		localize(
			"rbacMissingErrorAccountName",
			"Account Name: {0}\n",
			databaseAccount,
		),
		localize(
			"rbacMissingErrorSubscriptionName",
			"Subscription: {0}\n",
			subscription,
		),
	].join("");
	...
	const createUpdateSqlRoleAssignmentParameters: SqlRoleAssignmentCreateUpdateParameters =
		{
			...
			roleDefinitionId:
				fullAccountId + "/sqlRoleDefinitions/" + defaultRoleId,
			scope: fullAccountId,
		};
