...
export function registerDocDBCommands(): void {
	...
	registerCommandWithTreeNodeUnwrapping(
		"cosmosDB.createDocDBDocument",
		async (context: IActionContext, node?: DocDBDocumentsTreeItem) => {
			if (!node) {
			...
			const documentNode = <DocDBDocumentTreeItem>(
	...
	registerCommandWithTreeNodeUnwrapping(
		"cosmosDB.deleteDocDBDocument",
		async (context: IActionContext, node?: DocDBDocumentTreeItem) => {
			const suppressCreateContext: ITreeItemPickerContext = context;
			suppressCreateContext.suppressCreatePick = true;
			if (!node) {
				node = await pickDocDBAccount<DocDBDocumentTreeItem>(
					context,
...
export async function createDocDBDatabase(
	context: IActionContext,
	node?: DocDBAccountTreeItem
): Promise<void> {
	if (!node) {
		node = await pickDocDBAccount<DocDBAccountTreeItem>(context);
	}
	const databaseNode: DocDBDatabaseTreeItem = <DocDBDatabaseTreeItem>(
		await node.createChild(context)
	);
...
export async function deleteDocDBDatabase(
	context: IActionContext,
	node?: DocDBDatabaseTreeItem
): Promise<void> {
	const suppressCreateContext: ITreeItemPickerContext = context;
...
export async function deleteDocDBCollection(
	context: IActionContext,
...
async function pickDocDBAccount<T extends AzExtTreeItem>(
	context: IActionContext,
