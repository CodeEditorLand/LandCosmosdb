...
export class DocDBTriggersTreeItem extends DocDBTreeItemBase<TriggerDefinition> {
	...
	public async createChildImpl(
	...
	): Promise<DocDBTriggerTreeItem> {
		const client = this.root.getCosmosClient();
		const currTriggerList: AzExtTreeItem[] =
		...
		const triggerID = (
			await context.ui.showInputBox({
				...
				validateInput: (name: string) =>
					this.validateTriggerName(name, currTriggerNames),
			})
		).trim();
		...
		const response =
			await this.getContainerClient(client).scripts.triggers.create(body);
	...
	): QueryIterator<TriggerDefinition & Resource> {
		return this.getContainerClient(client).scripts.triggers.readAll(
			feedOptions,
		);
	}
	...
	private validateTriggerName(
		name: string,
		currTriggerNames: string[],
	): string | undefined {
		if (name.length < 1 || name.length > 255) {
			return localize(
				"nameLength",
				"Name has to be between 1 and 255 chars long",
			);
		}
...
export async function getTriggerType(
	...
	const triggerTypeOption =
		await context.ui.showQuickPick<vscode.QuickPickItem>(options, {
			placeHolder: localize(
				"createDocDBTriggerSelectType",
				"Select the trigger type",
			),
		});
	return triggerTypeOption.label === "Pre"
		? TriggerType.Pre
		: TriggerType.Post;
}

export async function getTriggerOperation(
	context: IActionContext,
): Promise<TriggerOperation> {
	const options = Object.keys(TriggerOperation).map((key) => ({
		label: key,
	}));
	const triggerOperationOption =
		await context.ui.showQuickPick<vscode.QuickPickItem>(options, {
			placeHolder: localize(
				"createDocDBTriggerSelectOperation",
				"Select the trigger operation",
			),
		});
	return TriggerOperation[
		triggerOperationOption.label as keyof typeof TriggerOperation
	];
}
