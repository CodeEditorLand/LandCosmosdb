 ...
 */
export abstract class DocDBAccountTreeItemBase extends DocDBTreeItemBase<
...
> {
	...
	constructor(
		...
		this.root = {
			...
			isEmulator,
			getCosmosClient: () =>
				getCosmosClient(endpoint, credentials, isEmulator),
		};
	...
	public get isServerless(): boolean {
		return this.databaseAccount?.capabilities
			? this.databaseAccount.capabilities.some(
					(cap) => cap.name === SERVERLESS_CAPABILITY_NAME,
				)
			: false;
	}
	...
	public async createChildImpl(
		...
		const database: DatabaseResponse = await client.databases.create({
			id: databaseName,
		});
				...
				if (this.root.isEmulator) {
					...
					try {
						...
						if (
							e instanceof Error &&
							isRbacException(e) &&
							!this.hasShownRbacNotification
						) {
							this.hasShownRbacNotification = true;

							const principalId =
								(await getSignedInPrincipalIdForAccountEndpoint(
									this.root.endpoint,
								)) ?? "";
							...
							if (
								...
								return await super.loadMoreChildrenImpl(
									clearCache,
								);
							} else {
								void showRbacPermissionError(
									this.fullId,
									principalId,
								);
							}
