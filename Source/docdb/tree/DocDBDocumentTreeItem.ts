 ...
 */
export class DocDBDocumentTreeItem
	...
	implements IEditableTreeItem
{
	...
	public async writeFileContent(
		...
		if (["_etag"].some((element) => !newData[element])) {
			throw new Error(
				`The "_self" and "_etag" fields are required to update a document`,
			);
		} else {
			// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
			...
			const response = await this.getDocumentClient(client).replace(
				newData,
				options,
			);
			// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
