...
export class DocDBStoredProceduresTreeItem extends DocDBTreeItemBase<StoredProcedureDefinition> {
	...
	): Promise<DocDBStoredProcedureTreeItem> {
		const client = this.root.getCosmosClient();
		const currStoredProcedureList: AzExtTreeItem[] =
		...
		const spID = (
			await context.ui.showInputBox({
				...
				validateInput: (name: string) =>
					this.validateStoredProcedureName(
						name,
						currStoredProcedureNames,
					),
			})
		).trim();
	...
	public getIterator(
		...
		return this.getContainerClient(client).scripts.storedProcedures.readAll(
			feedOptions,
		);
	}
	...
	private validateStoredProcedureName(
		name: string,
		currStoredProcedureNames: string[],
	): string | undefined {
		...
		if (currStoredProcedureNames.includes(name)) {
			return localize(
				"nameExists",
				'Stored Procedure "{0}" already exists.',
				name,
			);
		}
