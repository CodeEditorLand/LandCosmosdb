...
> {
	...
	public getIterator(
		...
		return client
			.database(this._database.id)
			.containers.readAll(feedOptions);
	}
	...
	public async createChildImpl(
		...
		const containerName = await context.ui.showInputBox({
			...
			validateInput: this.validateCollectionName.bind(this) as (
				name: string,
		...
		if (!this.parent.isServerless) {
			...
			const minThroughput = isFixed
				? minThroughputFixed
				: minThroughputPartitioned;
			...
			const throughput: number = Number(
				await context.ui.showInputBox({
					...
					validateInput: (input: string) =>
						validateThroughput(isFixed, input),
				}),
			);
	...
	protected async getNewPartitionKey(
...
function validateThroughput(
	...
	try {
		...
		if (
			value < minThroughput ||
			value > maxThroughput ||
			(value - minThroughput) % throughputStepSize !== 0
		) {
