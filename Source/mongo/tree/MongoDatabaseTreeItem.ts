...
const mongoExecutableFileName =
	process.platform === "win32" ? "mongo.exe" : "mongo";
...
export class MongoDatabaseTreeItem extends AzExtParentTreeItem {
	...
	public get description(): string {
		return ext.connectedMongoDB &&
			ext.connectedMongoDB.fullId === this.fullId
			? "Connected"
			: "";
	}
	...
	public async loadMoreChildrenImpl(
		...
		return collections.map(
			(collection) => new MongoCollectionTreeItem(this, collection),
		);
	}
	...
	public async connectToDb(): Promise<Db> {
		const accountConnection = await connectToMongoClient(
	...
	public async executeCommand(
		...
		if (command.collection) {
			...
			if (collection) {
				const collectionTreeItem = new MongoCollectionTreeItem(
					this,
					collection,
					command.arguments,
				);
				const result =
					await collectionTreeItem.tryExecuteCommandDirectly(command);
	...
	public async createCollection(
		...
		const db: Db = await this.connectToDb();
		const newCollection: Collection = await db.createCollection(
			collectionName,
			options,
		);
	...
	private async createShell(context: IActionContext): Promise<MongoShell> {
		...
		let shellPath: string | undefined = config.get(
			ext.settingsKeys.mongoShellPath,
		);
		const shellArgs: string[] = config.get(
			ext.settingsKeys.mongoShellArgs,
			[],
		);
		...
		const timeout =
			1000 *
			nonNullValue(
				config.get<number>(ext.settingsKeys.mongoShellTimeout),
				"mongoShellTimeout",
			);
		...
		if (!shellPathSetting) {
			...
			if (await cpUtils.commandSucceeds("mongo", "--version")) {
				...
				const openFile: vscode.MessageItem = {
					title: `Browse to ${mongoExecutableFileName}`,
				};
				const browse: vscode.MessageItem = {
				...
				if (response === openFile) {
					...
					while (true) {
						...
						if (baseName !== mongoExecutableFileName) {
							const useAnyway: vscode.MessageItem = {
								title: "Use anyway",
							};
							const tryAgain: vscode.MessageItem = {
								title: "Try again",
							};
							const response2 =
								await context.ui.showWarningMessage(
...
export function validateMongoCollectionName(
	collectionName: string,
