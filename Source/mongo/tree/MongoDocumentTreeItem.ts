...
export class MongoDocumentTreeItem
	...
	implements IEditableTreeItem
{
	...
	public static async update(
		...
		newDocument: IMongoDocument
	): Promise<IMongoDocument> {
		if (!newDocument._id) {
			throw new Error(
		...
		const result: UpdateWriteOpResult = await collection.replaceOne(
			filter,
			_.omit(newDocument, "_id")
		);
	...
	public async deleteTreeItemImpl(context: IActionContext): Promise<void> {
		...
		const deleteResult: DeleteWriteOpResultObject =
			await this.parent.collection.deleteOne({ _id: this.document._id });
		if (deleteResult.deletedCount !== 1) {
			throw new Error(
				`Failed to delete document with _id '${this.document._id}'.`
			);
		}
	}
