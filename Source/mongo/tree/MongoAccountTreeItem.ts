		...
		try {
			...
			mongoClient = await connectToMongoClient(
				...
				this.databaseAccount
					? nonNullProp(this.databaseAccount, "name")
					: appendExtensionUserAgent()
			);

			const databaseInConnectionString =
				getDatabaseNameFromConnectionString(this.connectionString);
			...
			return databases
				.filter(
					(database: IDatabaseInfo) =>
						!(
							database.name &&
							database.name.toLowerCase() === "admin" &&
							database.empty
						)
				) // Filter out the 'admin' database if it's empty
				.map(
					(database) =>
						new MongoDatabaseTreeItem(
							this,
							nonNullProp(database, "name"),
							this.connectionString
						)
				);
		} catch (error) {
