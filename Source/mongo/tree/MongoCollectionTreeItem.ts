...
import {
	MongoDocumentTreeItem,
	type IMongoDocument,
} from "./MongoDocumentTreeItem";

// eslint-disable-next-line @typescript-eslint/no-wrapper-object-types
type MongoFunction = (
	...args: (Object | Object[] | undefined)[]
) => Thenable<string>;
...
export class MongoCollectionTreeItem
	...
	implements IEditableTreeItem
{
	...
	public async writeFileContent(
		...
		const nodeInTree: MongoCollectionTreeItem | undefined =
			await ext.rgApi.appResourceTree.findTreeItem(this.fullId, context);
		...
		await Promise.all(
			nodesToRefresh.map((n) => n.refreshChildren(context, documents)),
		);
	...
	public async getFileContent(context: IActionContext): Promise<string> {
		const children = <MongoDocumentTreeItem[]>(
	...
	public async refreshChildren(
		...
		const documentNodes = <MongoDocumentTreeItem[]>(
			await this.getCachedChildren(context)
		);

		for (const doc of docs) {
			const documentNode = documentNodes.find(
				(node) => node.document._id.toString() === doc._id.toString(),
			);
	...
	): Promise<MongoDocumentTreeItem> {
		context.showCreatingTreeItem("");
		// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
		const result: InsertOneResult<MongoDocument> =
	...
	public async tryExecuteCommandDirectly(
		...
		| { deferToShell: true; result: undefined }
		| { deferToShell: false; result: string }
	> {
		...
		const parameters = command.arguments
			? command.arguments.map(parseJSContent)
			: [];

		const functions = {
			drop: new FunctionDescriptor(
				this.drop,
				"Dropping collection",
				0,
				0,
			...
			findOne: new FunctionDescriptor(
				this.findOne,
				"Finding document",
				0,
				2,
			...
			insertMany: new FunctionDescriptor(
				this.insertMany,
				"Inserting documents",
				1,
				2,
			...
			insertOne: new FunctionDescriptor(
				this.insertOne,
				"Inserting document",
				1,
				2,
			...
			deleteMany: new FunctionDescriptor(
				this.deleteMany,
				"Deleting documents",
				1,
				2,
			...
			deleteOne: new FunctionDescriptor(
				this.deleteOne,
				"Deleting document",
				1,
				2,
	...
	private async drop(): Promise<string> {
		try {
			...
			if (
				...
				error.code === NamespaceNotFoundCode
			) {
	...
	private async findOne(
		query?: Filter<MongoDocument>,
		fieldsOption?: MongoDocument,
	): Promise<string> {
		// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
		const result = await this.collection.findOne(query || {}, {
			projection: fieldsOption,
		});
	...
	private async insert(document: MongoDocument): Promise<string> {
		if (!document) {
			throw new Error(
				"The insert command requires at least one argument",
			);
		}
	...
	private async insertOne(
		...
		const insertOneResult: InsertOneResult<MongoDocument> =
	...
	private async insertMany(
		...
		assert.notEqual(
			documents.length,
			0,
			"Array of documents cannot be empty",
		);
		...
		// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
		const insertManyResult: InsertManyResult<MongoDocument> =
			await this.collection.insertMany(documents, insertManyOptions);
	...
	private async deleteOne(filter: Filter<MongoDocument>): Promise<string> {
		const deleteOneResult: DeleteResult =
	...
	private async deleteMany(filter: Filter<MongoDocument>): Promise<string> {
		const deleteOpResult: DeleteResult =
