...
export class MongoShell extends vscode.Disposable {
	...
	public static async create(
		...
		try {
			...
			const process: InteractiveChildProcess =
				await InteractiveChildProcess.create({
			...
			await shell.executeScript(
				`DBQuery.shellBatchSize = ${getBatchSizeSetting()}`,
			);
				...
				async (resolve, reject) => {
					try {
						...
						if ((<{ code?: string }>error).code === "EPIPE") {
							...
							error = new Error(
								"The process exited prematurely.",
							);
						}
