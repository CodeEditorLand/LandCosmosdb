...
import {
	stripQuotes,
	type MongoDatabaseTreeItem,
} from "./tree/MongoDatabaseTreeItem";
import {
	MongoDocumentTreeItem,
	type IMongoDocument,
} from "./tree/MongoDocumentTreeItem";

const notInScrapbookMessage =
	"You must have a MongoDB scrapbook (*.mongo) open to run a MongoDB command.";

export function getAllErrorsFromTextDocument(
...
export async function executeAllCommandsFromActiveEditor(
...
async function executeCommand(
	...
	if (command) {
		...
		if (command.name === "find" && !command.chained) {
			...
			const node = new MongoCollectionTreeItem(
				database,
				collection,
		...
		throw new Error(
			"No MongoDB command found at the current cursor location.",
		);
	}
}
...
async function refreshTreeAfterCommand(
	...
	if (command.name === "drop") {
		...
		command.collection &&
		command.name &&
		/^(insert|update|delete|replace|remove|write|bulkWrite)/i.test(
			command.name,
		)
	) {
...
class FindMongoCommandsVisitor extends MongoVisitor<MongoCommand[]> {
	...
	public visitCommand(ctx: mongoParser.CommandContext): MongoCommand[] {
		const funcCallCount: number = filterType(
			ctx.children,
	...
	private objectLiteralContextToObject(
		child: mongoParser.ObjectLiteralContext,
	): object {
		const propertyNameAndValue = findType(
			child.children,
	...
	private arrayLiteralContextToObject(
		child: mongoParser.ArrayLiteralContext,
	) {
		const elementList = findType(
			child.children,
		...
		if (elementList) {
			const elementItems = filterType(
				...
				mongoParser.PropertyValueContext,
			);
	...
	private functionCallContextToObject(
		...
		if (
			!(
				...
				argumentsToken._OPEN_PARENTHESIS
			)
		) {
	...
	private isodateToObject(
		...
		// eslint-disable-next-line @typescript-eslint/no-wrapper-object-types
		const date: Date | Object = this.tryToConstructDate(
			ctx,
			tokenText,
			true,
		);
	...
	private regexLiteralContextToObject(
		...
		try {
			...
			return {
				$regex: this.regexToStringNotation(pattern),
				$options: flags,
			};
		} catch (error) {
