...
import {
	executeAllCommandsFromActiveEditor,
	executeCommandFromActiveEditor,
	getAllErrorsFromTextDocument,
} from "./MongoScrapbook";
...
export function registerMongoCommands(): void {
	...
	registerCommandWithTreeNodeUnwrapping(
		...
		async (context: IActionContext, node?: MongoCollectionTreeItem) => {
			if (!node) {
	...
	registerCommandWithTreeNodeUnwrapping(
		...
		async (context: IActionContext, node?: MongoDatabaseTreeItem) => {
			if (!node) {
				// Include defaultExperience in the context to prevent https://github.com/microsoft/vscode-cosmosdb/issues/1517
				const experienceContext: ITreeItemPickerContext & {
					defaultExperience?: Experience;
				} = { ...context, defaultExperience: MongoExperience };
	...
	registerCommandWithTreeNodeUnwrapping(
		"cosmosDB.deleteMongoDocument",
		async (context: IActionContext, node?: MongoDocumentTreeItem) => {
			const suppressCreateContext: ITreeItemPickerContext = context;
			suppressCreateContext.suppressCreatePick = true;
			if (!node) {
				node = await pickMongo<MongoDocumentTreeItem>(
					context,
	...
	registerCommandWithTreeNodeUnwrapping(
		...
		async () => await vscodeUtil.showNewFile("", "Scrapbook", ".mongo")
	);
	registerCommandWithTreeNodeUnwrapping(
		"cosmosDB.executeMongoCommand",
		async (context: IActionContext, position?: vscode.Position) => {
...
function updateErrorsInScrapbook(
	context: IActionContext,
...
export async function deleteMongoCollection(
	context: IActionContext,
	node?: MongoCollectionTreeItem
): Promise<void> {
	const suppressCreateContext: ITreeItemPickerContext = context;
	suppressCreateContext.suppressCreatePick = true;
	if (!node) {
...
async function pickMongo<T extends AzExtTreeItem>(
	context: IActionContext,
