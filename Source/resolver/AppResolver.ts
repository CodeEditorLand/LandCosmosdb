	...
	public async resolveResource(
		...
		return await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				const subNode: AzExtParentTreeItem | undefined =
				...
				try {
					...
					switch (resource.type.toLowerCase()) {
						case resourceTypes[0]:
							const client = await createCosmosDBClient({
								...context,
								...subContext,
							});
							const databaseAccount =
								await client.databaseAccounts.get(
									resourceGroupName,
									name
								);
							dbChild =
							...
							return experience?.api === "MongoDB"
								? new ResolvedMongoAccountResource(
										dbChild as MongoAccountTreeItem,
										resource
								  )
						...
						case resourceTypes[2]:
							const postgresClient =
								resource.type.toLowerCase() === resourceTypes[1]
									...
									: await createPostgreSQLFlexibleClient({
											...context,
											...subContext,
									  });

							postgresServer = await postgresClient.servers.get(
								resourceGroupName,
								name
							);
							dbChild =
							...
							return new ResolvedPostgresServerResource(
								dbChild as PostgresServerTreeItem,
								resource
							);
						default:
							return null;
